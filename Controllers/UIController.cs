...
{
    private static readonly string sharedHead = @"
        <meta charset='utf-8'>
        <style>
    ...
    private static readonly string sharedScript = @"
        <script>
        ...
        <html><head>{sharedHead}</head><body>
            ...
            <script>
                async function load() {{
                    ...
                    const data = response.data || response; // Support pagination if data is nested
